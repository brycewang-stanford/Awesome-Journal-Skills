...
- A handling editor (the **Editor-in-Chief** — currently **Kris Byron** (Georgia State University), AMR EIC since 2023 succeeding Sherry M. B. Thatcher; verify the current slate on the AOM AMR editorial-leadership page — or an action editor) assigns the paper to several reviewers expert in your conversation. (Exact reviewer count and AMR's acceptance rate are **UNVERIFIED** — see source map; the rate is low, as for all top AOM journals.)
