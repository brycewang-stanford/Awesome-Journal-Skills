...
| 加速通道    | **PRP**：携带 AER/EMA/JPE/QJE/REStud 的拒稿决定信与审稿报告           | Article Type 选 'PRP'；不额外收费           |
| 投稿类型    | 常规、**短文（short）**、或 PRP                                     | 按贡献选择类型                             |
...
| 编辑匹配    | 建议匹配论文方向的 **(Co-)Editor**                                  | 贸易 vs 宏观/金融的分流很重要                 |

常规投稿录用率历史上约 **10-15%**，desk reject 约 **25%**（据 Co-Editor Costas Arkolakis 个人主页）。所有易变信息（现任编辑、确切投稿费、复制材料政策、篇幅限制）会随时间变化——请**到官方 Guide for Authors 核实**。部分 Elsevier/ScienceDirect 页面对自动抓取返回 HTTP 403，因此本包中若干条目标注为 **待核实**。
...
- [`resources/official-source-map.md`](resources/official-source-map.md) —— 本包每条事实背后的 JIE / Elsevier 官方 URL，未核实项标注 待核实
...
- 不断言易变的元数据（现任编辑、确切投稿费、复制材料规则、篇幅限制）——请到官网核实；部分条目标注 待核实
