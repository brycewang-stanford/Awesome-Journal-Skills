...
## Exhibit-placement decision table (article vs. supplementary, word-aware)

Because IO counts exhibits toward the cap and the supplementary file carries a ~20-page norm, every
exhibit needs a placement verdict tied to argument weight:
...
| Descriptive map / network of ties | only if structure is the point | main text if the structure *is* the argument |

The rule of thumb: if removing the exhibit would not weaken the core IR claim, it belongs in
supplementary material — that simultaneously respects the word cap and keeps the main text readable.

## Worked exhibit vignette (illustrative)

A treaty-compliance paper drafts five tables, with the headline monitoring-on-compliance result in a
dense six-column dyad-level table. The IO redesign converts it to one coefficient plot with 95% intervals,
...
magnitude and uncertainty in seconds. The four remaining tables (balance, full specs, two robustness
grids) move to supplementary material, keeping the main text under cap and the SI within the ~20-page
norm. Every plotted value is regenerated by the master script to match the package IO staff will re-run.
