...
Rule of thumb: if removing the exhibit would not weaken the core IR claim, it belongs in supplementary
material — which respects the word cap and keeps the main text readable.
...
the main text under cap and the SI within the ~20-page norm. Every plotted value is regenerated by the
master script to match the package IO staff will re-run.
