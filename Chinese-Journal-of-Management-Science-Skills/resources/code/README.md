<!-- 自 Economic-Research-Journal-Skills/resources/code 引入（Stata 18 MP 已实跑验证，2026-06），用于 Chinese-Journal-of-Management-Science-Skills 自包含，服务本包金融工程实证与政策冲击支线。命令以规范源为准，勿在此处擅改。 -->
# 代码模板库（Stata + Python）
...
> 安装统一用 `ssc install <pkg>, replace`。各方法的报告口径与反模式见对应 `cjms-*` 技能。
>
> 本目录给可运行的端到端 `.do` 流程；每个命令的来源与权威引用请在对应 `cjms-*` 技能正文与
> `../official-source-map.md` 中查证。
