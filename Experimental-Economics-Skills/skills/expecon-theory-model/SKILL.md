...
description: Use when working on theory and model craft for a Experimental Economics manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Theory and Model Craft (expecon-theory-model)

## When to trigger
- The manuscript is aimed at **Experimental Economics (Experimental Economics)** and theory and model craft is the active bottleneck.
...
5. End with a handoff to `expecon-robustness` if the stage passes, or back to `expecon-workflow` if it does not.
