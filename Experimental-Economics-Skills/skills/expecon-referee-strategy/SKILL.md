...
description: Use when working on referee strategy for a Experimental Economics manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Referee Strategy (expecon-referee-strategy)

## When to trigger
- The manuscript is aimed at **Experimental Economics (Experimental Economics)** and referee strategy is the active bottleneck.
...
5. End with a handoff to `expecon-submission` if the stage passes, or back to `expecon-workflow` if it does not.
