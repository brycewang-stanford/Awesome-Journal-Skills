...
description: Use when working on tables and figures for a Experimental Economics manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Tables and Figures (expecon-tables-figures)

## When to trigger
- The manuscript is aimed at **Experimental Economics (Experimental Economics)** and tables and figures is the active bottleneck.
...
5. End with a handoff to `expecon-writing-style` if the stage passes, or back to `expecon-workflow` if it does not.
