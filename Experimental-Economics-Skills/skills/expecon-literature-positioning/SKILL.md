...
description: Use when working on literature positioning for a Experimental Economics manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Literature Positioning (expecon-literature-positioning)

## When to trigger
- The manuscript is aimed at **Experimental Economics (Experimental Economics)** and literature positioning is the active bottleneck.
...
5. End with a handoff to `expecon-identification` if the stage passes, or back to `expecon-workflow` if it does not.
