...
description: Use when working on theory and model craft for a IMF Economic Review manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Theory and Model Craft (imfer-theory-model)

## When to trigger
- The manuscript is aimed at **IMF Economic Review (IMFER)** and theory and model craft is the active bottleneck.
...
5. End with a handoff to `imfer-robustness` if the stage passes, or back to `imfer-workflow` if it does not.
