...
description: Use when working on referee strategy for a IMF Economic Review manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Referee Strategy (imfer-referee-strategy)

## When to trigger
- The manuscript is aimed at **IMF Economic Review (IMFER)** and referee strategy is the active bottleneck.
...
5. End with a handoff to `imfer-submission` if the stage passes, or back to `imfer-workflow` if it does not.
