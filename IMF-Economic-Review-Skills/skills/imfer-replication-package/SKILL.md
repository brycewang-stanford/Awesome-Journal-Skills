...
description: Use when working on replication package for a IMF Economic Review manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Replication Package (imfer-replication-package)

## When to trigger
- The manuscript is aimed at **IMF Economic Review (IMFER)** and replication package is the active bottleneck.
...
5. End with a handoff to `imfer-referee-strategy` if the stage passes, or back to `imfer-workflow` if it does not.
