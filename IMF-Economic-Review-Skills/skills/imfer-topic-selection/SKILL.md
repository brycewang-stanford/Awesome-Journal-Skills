...
description: Use when working on topic selection for a IMF Economic Review manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Topic Selection (imfer-topic-selection)

## When to trigger
- The manuscript is aimed at **IMF Economic Review (IMFER)** and topic selection is the active bottleneck.
...
5. End with a handoff to `imfer-literature-positioning` if the stage passes, or back to `imfer-workflow` if it does not.
