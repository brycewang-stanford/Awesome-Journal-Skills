...
description: Use when working on identification strategy for a IMF Economic Review manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Identification Strategy (imfer-identification)

## When to trigger
- The manuscript is aimed at **IMF Economic Review (IMFER)** and identification strategy is the active bottleneck.
...
5. End with a handoff to `imfer-theory-model` if the stage passes, or back to `imfer-workflow` if it does not.
