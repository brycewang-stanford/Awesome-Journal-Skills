...
description: Use when working on writing style for a IMF Economic Review manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Writing Style (imfer-writing-style)

## When to trigger
- The manuscript is aimed at **IMF Economic Review (IMFER)** and writing style is the active bottleneck.
...
5. End with a handoff to `imfer-replication-package` if the stage passes, or back to `imfer-workflow` if it does not.
