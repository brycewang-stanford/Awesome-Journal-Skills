...
description: Use when working on topic selection for a World Development manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Topic Selection (worlddev-topic-selection)

## When to trigger
- The manuscript is aimed at **World Development (World Development)** and topic selection is the active bottleneck.
...
5. End with a handoff to `worlddev-literature-positioning` if the stage passes, or back to `worlddev-workflow` if it does not.
