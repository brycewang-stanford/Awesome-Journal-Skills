---
name: ijoc-methods
description: Use when working on methods for a INFORMS Journal on Computing manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Methods (ijoc-methods)

## When to trigger
- The manuscript is aimed at **INFORMS Journal on Computing (IJOC)** and methods is the active bottleneck.
...
5. End with a handoff to `ijoc-data-analysis` if the stage passes, or back to `ijoc-workflow` if it does not.
