...
description: Use when working on writing style for a The World Bank Economic Review manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Writing Style (wber-writing-style)

## When to trigger
- The manuscript is aimed at **The World Bank Economic Review (WBER)** and writing style is the active bottleneck.
...
5. End with a handoff to `wber-replication-package` if the stage passes, or back to `wber-workflow` if it does not.
...
【Next skill】wber-replication-package
```
