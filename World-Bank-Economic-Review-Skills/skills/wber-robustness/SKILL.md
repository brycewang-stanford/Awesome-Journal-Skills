...
description: Use when working on robustness strategy for a The World Bank Economic Review manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Robustness Strategy (wber-robustness)

## When to trigger
- The manuscript is aimed at **The World Bank Economic Review (WBER)** and robustness strategy is the active bottleneck.
...
5. End with a handoff to `wber-tables-figures` if the stage passes, or back to `wber-workflow` if it does not.
...
【Next skill】wber-tables-figures
```
