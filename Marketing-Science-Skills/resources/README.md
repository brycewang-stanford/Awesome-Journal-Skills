# Marketing Science — Resources

Capability layer for the **Marketing Science** skill pack. The runnable code is vendored for
...
| [`worked-examples/01-introduction.md`](worked-examples/01-introduction.md) | Before→after rewrite of a (fictional) *Marketing Science*-style **introduction**: lead with the marketing question and the counterfactual a model delivers, demote the estimator to evidentiary backbone, name the contribution on one primary dimension, close with the managerial implication. |
| [`exemplars/library.md`](exemplars/library.md) | Real, web-verified *Marketing Science* papers by topic × method, with a sibling-journal omission guard. Design positioning only — read the originals before citing numbers. |
