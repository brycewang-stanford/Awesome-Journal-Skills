...
description: Use when working on topic selection for a International Economic Review manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Topic Selection (ier-topic-selection)

## When to trigger
- The manuscript is aimed at **International Economic Review (IER)** and topic selection is the active bottleneck.
...
5. End with a handoff to `ier-literature-positioning` if the stage passes, or back to `ier-workflow` if it does not.
