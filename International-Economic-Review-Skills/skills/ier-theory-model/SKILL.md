...
description: Use when working on theory and model craft for a International Economic Review manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Theory and Model Craft (ier-theory-model)

## When to trigger
- The manuscript is aimed at **International Economic Review (IER)** and theory and model craft is the active bottleneck.
...
5. End with a handoff to `ier-robustness` if the stage passes, or back to `ier-workflow` if it does not.
