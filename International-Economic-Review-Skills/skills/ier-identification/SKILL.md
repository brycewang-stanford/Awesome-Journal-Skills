...
description: Use when working on identification strategy for a International Economic Review manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Identification Strategy (ier-identification)

## When to trigger
- The manuscript is aimed at **International Economic Review (IER)** and identification strategy is the active bottleneck.
...
5. End with a handoff to `ier-theory-model` if the stage passes, or back to `ier-workflow` if it does not.
