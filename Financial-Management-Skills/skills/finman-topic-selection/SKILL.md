...
description: Use when working on topic selection for a Financial Management manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Topic Selection (finman-topic-selection)

## When to trigger
- The manuscript is aimed at **Financial Management (FM)** and topic selection is the active bottleneck.
...
5. End with a handoff to `finman-literature-positioning` if the stage passes, or back to `finman-workflow` if it does not.
