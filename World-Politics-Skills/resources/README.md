...
|---|---|
| [`worked-examples/`](worked-examples/) | Before→after rewrites in *World Politics* house style. Start with [`01-introduction.md`](worked-examples/01-introduction.md) — a fictional comparative-politics/IR introduction reworked to the journal's "travels + theory-forward + contribution front-loaded" arc, mapped to this pack's skills. |
| [`exemplars/library.md`](exemplars/library.md) | Real, **web-verified** *World Politics* (Cambridge UP) articles by method × topic, each confirmed on Cambridge Core, with a sibling-confusion guard (not IO / International Security / APSR / CPS / Comparative Politics). Design positioning only — no invented findings. |
