# Journal of Finance — Resources

Capability layer for the **Journal of Finance** skill pack. The runnable code is vendored for
...
| [`worked-examples/01-introduction.md`](worked-examples/01-introduction.md) | See a before→after rewrite of an empirical-finance paper introduction in JF house style (question + answer + economic magnitude up front, identification named, contribution early). Illustrative fictional paper. |
| [`worked-examples/02-execution-walkthrough.md`](worked-examples/02-execution-walkthrough.md) | **The execution bridge, run end to end.** A staggered-DiD identification taken through real StatsPAI MCP calls (`detect_design → recommend → callaway_santanna → TWFE contrast → sun_abraham → audit_result → honest_did`) — synthetic data, but every number is an actual tool return. Shows TWFE bias (−0.0227 vs clean −0.0272), flat pre-trends (p=0.155), and an honest sensitivity breakdown point. |
| [`exemplars/library.md`](exemplars/library.md) | Benchmark against **real, web-verified Journal of Finance papers** organized by method × topic, with a sibling-journal (JFE/RFS/JFQA) misattribution guard. Design positioning only — no fabricated numbers. |
| [execution-with-mcp](../../shared-resources/empirical-methods/execution-with-mcp.md) | **Guidance → a fitted, audited result.** Maps each design family / reviewer objection to the concrete StatsPAI / Stata MCP tools in this environment, so `jf-identification` and `jf-robustness` *run* the modern DiD / weak-IV-robust CI / multiple-testing correction and report the number — not just recommend it. |
