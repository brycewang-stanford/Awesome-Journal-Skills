...
description: Use when working on replication package for a Journal of Health Economics manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Replication Package (jhe-replication-package)

## When to trigger
- The manuscript is aimed at **Journal of Health Economics (JHE)** and replication package is the active bottleneck.
...
5. End with a handoff to `jhe-referee-strategy` if the stage passes, or back to `jhe-workflow` if it does not.
