...
### 3.3 scorecard 已饱和 —— 静态尺子退役预告
min = mean = max = 94.0 后，该尺子对后续质量工作失去区分度。已在 ROADMAP 留档：
下月起质量牵引应转向 `tools/skillopt/` 的 rollout→judge→validation-gate 行为
评测路线；静态 scorecard 保留为回归防线（防跌破 94），不再作为提升目标。

### 3.4 category-8 仍在 10/100
Wave 25-A 下一批（`.maintenance/ASSET-INVENTORY.md` 已更新）：The Web
Conference / WSDM / SIGIR / CVPR。按「每波过全部硬门槛」的节奏滚动，不冲量。
