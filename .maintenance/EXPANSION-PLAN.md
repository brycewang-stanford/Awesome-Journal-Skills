...
- AJS 现状（2026-06-01）：**1036 skills / 48 包**（README 徽章）。其中
  `Computer-Science-Conference-Skills` 是 AI-first CS 会议**广度包**，`NeurIPS-Skills`
  `ICML-Skills` 和 `ICLR-Skills` 是 category 8 的前三个完整会议深度包；广度包仍不是终态
  "一会一完整 12-skill 深度包"的替代。
...
- **Wave 1**：去重盘点 — 把现有 48 包 / 200 根卡映射进 10 大类，标出"已有/需补"。
- **Wave 2+**：按大类滚动建包。优先级：
