...
### Week 2 增补（2026-07-08，owner 指示「把下一步工作全部做完」后提前执行）

- [x] 扩张第二波：The Web Conference / WSDM / SIGIR / CVPR 四个完整 12-skill
      深度包（Wave 25-A 顺序 11–14），同一金标准与事实纪律（官方 2026 周期
      锚点 + 搜索渲染访问方式披露 + 待核实清单）。category-8 完成度 10→14/100，
      仓库总量 199→203 pack / 2950→2998 skill，全部对账同 commit。
- [x] source-map 搜索渲染清欠（top-5 包）：8 项「待核实」升级为「搜索快照
      核实（直连受阻）」层级（PNAS-Nexus 在任 EIC/定位语言/双许可证菜单、
      Econometrica 投稿平台冲突裁定 + 官方年报计数、中国工业经济零收费/唯一
      投稿入口/初审≤30 工作日、世界经济基金标注位置、管理科学学报主管单位），
      其余未决项全部转为带日期、带检索记录的精确债务。注：report-only 计数器
      因披露词命中反而上升——已留档为测量学现象，本月不调正则。
- [x] 质量换挡落地：run_checks scorecard 硬门槛 90→94（回归防线）；
      QUALITY-LANE-2026-09.md 轻量方案；撤回 skillopt 复活建议（owner 已于
      2026-06-29 刻意拆除该装置，commit b9c8fe75）。
- [x] v1.1.0 双语 release notes 草案：`.maintenance/RELEASE-NOTES-v1.1.0.md`
      （owner 验收后可直接发布；tag 不代打）。
- [x] 三项遗留决策全量执行（owner 授权，2026-07-08）：①PR #6 已创建并在
      CI 绿灯后合并进 main（merge commit f8dd60e）；v1.1.0 tag 因会话凭证仅限
      分支推送而留给 owner 一键执行（本地标签与双语 notes 均已备好）。
      ②CI 侧 live 复核通道交付：tools/live_check_fetch.py + LIVE-CHECK-URLS.txt
      （15 个未决项官方 URL）；包装 workflow 因凭证缺 workflows 权限（git 与
      API 双通道 403）暂存 .maintenance/live-check.workflow.yml，owner 一条
      命令激活。③EN core-61 第二梯队 8 包（NAACL / ECCV / PLDI / SIGMOD /
      STOC / NSDI / USENIX Security / NDSS）建成并对账：213→221 pack /
      3118→3214 skill，category-8 24→32/100；全部硬检查复绿（min = mean =
      94.0、克隆审计零命中，姊妹会交叉相似度 NAACL↔ACL/EMNLP ≤0.113、
      ECCV↔CVPR/ICCV ≤0.070、STOC↔COLT ≤0.061、NDSS/USENIX-Sec↔S&P/CCS
      ≤0.091）；双语 banner 随终态数字重渲染。本批归入 v1.2.0 周期。
- [x] EN core-61 第三梯队 8 包（owner 指示继续，2026-07-08）：FOCS / SODA /
      POPL / OOPSLA / VLDB / CIKM / EuroSys / ASPLOS 建成并对账：221→229 pack /
      3214→3310 skill，category-8 32→40/100。时效性亮点：POPL 2027 截稿日为
      2026-07-09（次日）已内置警告；三处简报假设被代理证伪并如实纠正
      （POPL 25 页上限 + 2023 起全双盲；VLDB 2026 在波士顿、单盲、当前开放
      Vol 20）。姊妹会交叉相似度全部达标（POPL/OOPSLA↔PLDI ≤0.20、
      FOCS/SODA↔STOC/COLT、VLDB↔SIGMOD ≤0.10、EuroSys↔SOSP/OSDI/NSDI
      ≤0.074、CIKM↔SIGIR/WSDM/KDD）。全部硬检查复绿；banner 随终态重渲染；
      本批归入 v1.2.0。再下一批：UIST / CSCW / ISCA / MICRO / RSS / CoRL /
      INTERSPEECH / COLM 等。
- [x] 扩张第三波（owner 再次指示继续推进，2026-07-08）：ICCV / ACL / EMNLP /
      ICRA 四个完整 12-skill 深度包（Wave 25-A 顺序 15–18）。category-8
      14→18/100，仓库总量 203→207 pack / 2998→3046 skill。同标准验证：
      audit 3046/207 通过、全仓克隆审计 0.75 零命中（ICCV↔CVPR 相似度
      0.017–0.061，EMNLP↔ACL 独立成文无同构对）、scorecard 207 包
      min = mean = 94.0。ARR 两段制（ACL/EMNLP）、ICCV 双年节奏与
      CVPR/ECCV 三角、ICRA 的 RA-L 双通道决策与 2026 转双匿名等
      venue-true 特质全部落地；ICRA 2027 / ICCV 2027 / SIGIR 2027 等
      未发布周期一律 待核实。下一批队列：CHI / SOSP / OSDI / IEEE S&P。
- [x] 扩张第四波（收官，owner 指示「还有什么值得做的都做完」，2026-07-08）：
      CHI / SOSP / OSDI / IEEE S&P / ACM CCS / ICSE 六个完整 12-skill 深度包
      （Wave 25-A 顺序 19–24）——**启动批次 24/24 全部完成**。category-8
      18→24/100，仓库总量 207→213 pack / 3046→3118 skill。建设过程中遭遇
      会话额度中断（6 个代理同时终止），额度重置后全部续建完成，半成品
      零丢弃。同标准验证与对账（数字见 ACCEPTANCE 增补）。
- [x] 首页双语 banner 重渲染（2026-07-08）：原 PNG 烧录数字停留在
      2137 skill / 127 pack 世代，且覆盖条分段（115+100+154+135=504）与
      README 口径 519 冲突。更新四个主数字为终态（3118 / 213 / 519 / 155），
      按事实纪律移除无法为 519 派生拆分的分段数字标签（保留比例视觉），
      用 assets/banner-src/render_banners.mjs 重渲染双语 PNG。
