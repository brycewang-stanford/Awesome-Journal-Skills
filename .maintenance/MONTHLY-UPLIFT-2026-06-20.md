...
| 2026-06-23 | Weekly safe uplift: unclaimed routing + JPART design floor | done | Avoided current claim-sensitive low-tail packs; strengthened `Agriculture-Environment-Journal-Skills` routing gates/examples and JPART theory-design diagnostics. `quality_scorecard.py --json` -> JPART 93.8 to 94.0, Agriculture breadth stays 92.0 with stronger routing materials; `skillopt_gate.py gate --baseline /tmp/ajs-agrienv-baseline.json` PASS; counts remain 2883/193/200; source-map/root warnings 0; clone fail hits 0; `run_checks.py --skip-reports` and `git diff --check` clean. No push because `.maintenance/CLAIMS.md` currently says parallel edits should not be pushed. |
| 2026-06-23 | Weekly safe uplift batch 2: PAR transparency/design floor | done | Chose `Public-Administration-Review-Skills` after re-reading claims; avoided engineering/agriculture/humanities/clinical breadth candidates because adjacent category lanes or medicine boundaries raised conflict risk. Added PAR-specific transparency package gates, restricted-data decision tree, reproducibility smoke test, design-fit gate, practice-safe inference rules, and reviewer stress tests. `quality_scorecard.py --json` -> PAR 93.8 to 94.0; `skillopt_gate.py gate --baseline /tmp/ajs-weekly-batch2-baseline.json` PASS; counts remain 2883/193/200; source-map/root warnings 0; clone fail hits 0; `run_checks.py --skip-reports` and `git diff --check` clean; no push under active claims-board no-push rule. |
