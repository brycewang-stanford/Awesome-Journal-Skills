...
> 仅保留相对持久的规范。主编、费用、各 Department 编辑名单、确切的页数/摘要限制都会变化 —— 真正投稿前务必在官方 M&SOM 投稿指南页与 INFORMS 体例文件上核验。无法通过直接抓取确认的条目，统一标注为 **待核实**。
...
| 摘要            | **结构化、≤ 300 词**，四个必填部分                          | "管理含义"是必填章节，而非可选包装                         |
...
| `msom-contribution-framing`   | 明确的运营洞见 + 四段式结构化摘要                                       |
...
- [`resources/official-source-map.md`](resources/official-source-map.md) —— 每条事实背后的官方 INFORMS/M&SOM 链接（访问于 2026-06-01）
...
- **强制四段式结构化摘要**（Problem definition / Methodology / Results / Managerial implications），≤ 300 词、无术语堆砌。
...
| 投稿费             | 免费（待核实）                         | $79（2025 年 8 月起）       | INFORMS                        | 须核实                       |
