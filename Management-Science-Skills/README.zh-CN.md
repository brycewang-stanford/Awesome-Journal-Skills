...
| 契合度         | 须属于本刊，而非姊妹刊 **Operations Research / M&SOM / Marketing Science** | 约 50–60%（估计）稿件被初筛拒，常因契合度                 |
| 评审           | **双盲匿名**；约 2–3 位评审，常由副编辑协调（估计）                | 彻底匿名；会议论文版本须匿名披露                            |
| 透明度         | **数据与代码披露政策** —— 数据编辑在发表前**核验**可复现性          | 第一天起就构建可一键重生结果的复现包                        |
...
- [`resources/official-source-map.md`](resources/official-source-map.md) —— 每条已核实事实背后的 INFORMS/《管理科学》官方网址（访问于 2026-06-01）
...
- **透明度：** **数据与代码披露政策**（2019-07-01 起投稿适用）；**数据编辑**在发表前**核验**正文结果可复现（平均增加约 17 天）。

详见 [`resources/official-source-map.md`](resources/official-source-map.md)，其中含标注为 **待核实** 的条目。
