...
| [`worked-examples/`](worked-examples/) | Before→after rewrite of a strategy empirical-paper **introduction** in SMJ house style, derived from this pack's contribution-framing / literature-positioning / writing-style skills. Clearly-marked **fictional** illustrative paper; teaches the conversation → gap → "our move" → design → contribution-to-strategy-theory arc. |
| [`exemplars/library.md`](exemplars/library.md) | **Web-verified real SMJ papers** by theme × method (RBV, dynamic capabilities, variance decomposition, alliance learning, formal simulation, quasi-experiment), each confirmed as SMJ-specific with a sibling-venue do-not-misattribute guardrail. Design positioning only — zero invented findings. |
