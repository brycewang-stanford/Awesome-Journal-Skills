# Population and Development Review — Official Source Map

Primary and authoritative sources behind the PDR-specific facts injected into this skill pack. Access
date for all entries: **2026-06** (检索于 2026-06；以官网为准). Re-verify volatile items (current
editors and editorial term, exact abstract/length caps, submission portal link, fee and Online Open
options, data-policy wording, Free Format scope) on the official page before relying on them — journal
pages change.

## Official journal, author & submission pages

- **https://onlinelibrary.wiley.com/journal/17284457**
  — *Population and Development Review* on Wiley Online Library (publisher home, current issues).
  Verifies: published by **Wiley** on behalf of the **Population Council**; quarterly; ISSN
  1728-4457 (online).
- **https://onlinelibrary.wiley.com/page/journal/17284457/homepage/forauthors.html**
  — Author Guidelines. Verifies: electronic submission via **ScholarOne Manuscripts**; **Free Format**
  first-round submission (references in any consistent style, strict formatting deferred to revision);
  required author fees for submission and publication: **none** under the standard model; data sharing
  / data availability statement under Wiley's framework. Open the live page for exact abstract,
  length-cap, and Online Open APC wording.
- **https://popcouncil.org/population-and-development-review-journal/**
  — Population Council's PDR page. Verifies: the journal "seeks to advance knowledge of the
  relationships between **population and social, economic, and environmental change**" and addresses
  related **policy** issues; **quarterly**; **Wiley publishes on behalf of the Population Council**;
  article types include **research articles, policy analysis, sociographic/critical pieces, notes and
  commentaries, data interpretation, book reviews, and archival materials**; complimentary subscriptions
  for qualified applicants in developing countries.

## Scope, history & owner

- **https://en.wikipedia.org/wiki/Population_and_Development_Review**
  — Verifies: **founded 1975**; quarterly peer-reviewed journal published by **Wiley-Blackwell** on
  behalf of the **Population Council**; covers the interrelationships between **population and
  socioeconomic development** and related **public policy**.
- **https://popcouncil.org/insight/pdr50-preface/** — "Celebrating 50 years of Population and
  Development Review." Context for the 1975 founding and the journal's history/mission.
- **https://www.jstor.org/journal/popudeverevi** and **https://www.jstor.org/stable/i307469**
  — JSTOR archive. Verifies: PDR back to **Vol. 1 (1975)**; **founding editor Paul Demeny**.
- **https://onlinelibrary.wiley.com/pb-assets/assets/17284457/INDEX47-1642714665837.pdf**
  — PDR **Cumulative Contents Volumes 1–47 (1975–2021)** (official Wiley index). The authoritative
  record for confirming whether a paper appeared in PDR and its volume/issue/pages.

## Review process

- Verifies (from the author guidelines and secondary corroboration): review is **double-anonymized** —
  authors must **remove all identifying information about all co-authors**; an **editorial committee**
  decides whether to send a paper out; if sent for review, it goes to **at least two referees**; authors
  can expect written comments and a decision **approximately three months** after submission. Confirm
  current categories and timelines on the official author page.

## Bounded live-check rules

- Treat the Wiley author-guidelines page as controlling for submission mechanics; re-open it before
  quoting exact abstract or per-type length limits.
- Use **Raya Muttarak and Joshua Wilde** as editor names only when the Population Council/Wiley masthead
  is live; Paul Demeny remains historical founding-editor context.
- Free Format is safe to encode for first-round routing, but final style and file requirements should
  be taken from the live Wiley author page.
- Standard submission/publication under the non-OA route has no author fee in this map. Optional Online
  Open charges are a Wiley post-acceptance choice and should be checked only if selected.
- Use ScholarOne Manuscripts as the working submission system but follow the live Wiley "Submit" link
  for the exact portal URL.
- Data-availability statements follow Wiley's framework; do not overstate the policy as a verified
  pre-publication replication gate.
- Do not encode desk-rejection rates or decision-time distributions as fixed policy.
