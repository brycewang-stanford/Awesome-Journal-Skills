...
description: Use when working on literature positioning for a Human Relations manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Literature Positioning (humrel-literature-positioning)

## When to trigger
- The manuscript is aimed at **Human Relations (Human Relations)** and literature positioning is the active bottleneck.
...
5. End with a handoff to `humrel-methods` if the stage passes, or back to `humrel-workflow` if it does not.
