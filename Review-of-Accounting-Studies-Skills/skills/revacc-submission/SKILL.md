...
description: Use when working on submission preflight for a Review of Accounting Studies manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Submission Preflight (revacc-submission)

## When to trigger
- The manuscript is aimed at **Review of Accounting Studies (RAST)** and submission preflight is the active bottleneck.
...
5. End with a handoff to `revacc-review-process` if the stage passes, or back to `revacc-workflow` if it does not.
