...
Default assumption: unless the user says otherwise, treat the target as EJ. Operational tells that you are at EJ and not a sibling: submission is through the **Editorial Express** portal (not ScholarOne); review is **single-blind** (referees are anonymous to authors; authors are *not* anonymized) (检索于 2026-06；以官网为准); there is an explicit **short-paper option in the AER:Insights style (<6,000 words, ~5 exhibits)** alongside full-length articles; and accepted empirical papers go through an **EJ Data Editor reproducibility check** with the replication package deposited to **Zenodo** under the RES policy (DCAS-endorsed). Editor-in-chief as of 2026: **Francesco Lippi** (检索于 2026-06；以官网为准). Re-verify volatile specifics on the official OUP/RES pages.
