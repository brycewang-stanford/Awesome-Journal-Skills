---
name: jfm-robustness
description: Use when working on robustness strategy for a Journal of Financial Markets manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Robustness Strategy (jfm-robustness)

## When to trigger
- The manuscript is aimed at **Journal of Financial Markets (JFM)** and robustness strategy is the active bottleneck.
...
5. End with a handoff to `jfm-tables-figures` if the stage passes, or back to `jfm-workflow` if it does not.
