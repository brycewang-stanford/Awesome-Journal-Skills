...
description: Use when working on topic selection for a Journal of Economic Geography manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Topic Selection (jegeo-topic-selection)

## When to trigger
- The manuscript is aimed at **Journal of Economic Geography (JEG)** and topic selection is the active bottleneck.
...
5. End with a handoff to `jegeo-literature-positioning` if the stage passes, or back to `jegeo-workflow` if it does not.
