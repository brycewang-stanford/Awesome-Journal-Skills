# Information Systems Research — Resources

Capability layer for the **Information Systems Research** skill pack. The runnable code is vendored for
...
|---|---|
| [`code/`](code/) | Reproducible Stata + Python causal-inference skeleton (clean → descriptive → DiD/IV/RDD/DML → mechanism → robustness → tables). Adapted from the verified Economic-Research-Journal-Skills library (Stata 18 MP, 2026-06); copy-and-adapt, change no command blindly. In ISR this is **supporting infrastructure** for the empirical genre — the IT artifact and sociotechnical mechanism lead, the estimator follows. |
| [`worked-examples/01-introduction.md`](worked-examples/01-introduction.md) | See a before→after rewrite of an IS empirical-paper introduction in ISR house style (front-load the IS contribution, IT-artifact centrality, the intradisciplinary bridge). Illustrative fictional paper. |
| [`exemplars/library.md`](exemplars/library.md) | Benchmark against **real, web-verified ISR papers** organized by genre × method × topic. Design positioning only — no fabricated numbers; includes a sibling-journal (MISQ/JMIS/JAIS/Management Science) guardrail. |
