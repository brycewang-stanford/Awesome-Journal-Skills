...
The evaluation scores that same code path. On a 1,738-paper gold set split into a tuning half and a held-out half, a **bare title** — the most pessimistic way to ask — puts the true venue in the top 10 for **62.0%** of papers. Pasting the abstract, which is what an author actually does, gives **67.5%**, and supplying the discipline **74.4%** ([`eval/RESULTS.md`](shared-resources/journal-selection/eval/RESULTS.md)). Every venue carries **two vocabularies**: one from the prose this repository wrote about it, one from the titles of the articles it actually published. The second was added in August 2026 and moves the bare-title figure on its own from 46.7% to 62.0%, cutting the share of papers whose true venue is unreachable at any depth from 15% to 5%. Retrieval only says *which packs to read*; the recommendation still comes from opening each venue's own pack and source-map.
