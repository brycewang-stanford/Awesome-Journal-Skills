...
description: Use when working on topic selection for a Journal of Money, Credit and Banking manuscript. Provides journal-specific decision checks and handoff criteria; it does not invent evidence or citations.
---

# Topic Selection (jmcb-topic-selection)

## When to trigger
- The manuscript is aimed at **Journal of Money, Credit and Banking (JMCB)** and topic selection is the active bottleneck.
...
5. End with a handoff to `jmcb-literature-positioning` if the stage passes, or back to `jmcb-workflow` if it does not.
